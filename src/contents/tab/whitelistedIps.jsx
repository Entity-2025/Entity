...
import { EntityManageWhitelist } from "@/components/title/EntityTitle";

export default function WhitelistedIpsTab({ user }) {
	...
	const fetchShortlinks = async () => {
		...
		try {
			...
			if (data?.shortlinks?.length) {
				...
				setWhitelist(data.shortlinks[0].ipWhitelist || []);
			}
		} catch (err) {
	...
	const handleSelectChange = (key) => {
		...
		setWhitelist(found?.ipWhitelist || []);
	};
	...
	const saveWhitelist = async (ips) => {
		...
		try {
			...
			toast.success("[SUCCESS] Whitelist Updated!");
		} catch (err) {
	...
	const addIps = () => {
		...
		const updated = Array.from(new Set([...whitelist, ...ips]));
		setInputValue("");
	...
	const removeIp = (ip) => {
		const updated = whitelist.filter((item) => item !== ip);
		saveWhitelist(updated);
	};
	...
	useEffect(() => {
		...
		if (found) setWhitelist(found.ipWhitelist || []);
			...
			<div className="w-full bg-white dark:bg-neutral-900 shadow-lg rounded-2xl p-6 flex flex-col gap-6">
				{shortlinks.length === 0 && !loading ? (
					...
					<>
						...
						<div>
							<h3 className="text-sm font-medium mb-3">Whitelisted IPs</h3>
