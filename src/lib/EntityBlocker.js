...
const BLOCKED_ASN_PATTERNS = [
    ...
    "VIDEOTRON LTEE", "M EUROPE SRL", "MIDCONTINENT COMMUNICATIONS",
    "ASIANET SATELLITE COMMUNICATIONS PVT LTD", "SHARKTECH", "DEDICATED.COM",
    "WEB OBJECTS LLC"
];
...
export function EntityAsnCheck(shortlink, visitorIp, asnInfo) {
    if (!asnInfo?.org) return null;
    const orgUpper = asnInfo.org.toUpperCase();

    if (BLOCKED_ASN_PATTERNS.some(p => orgUpper.includes(p))) {
        // return EntityBlock(shortlink, 'Your ASN is banned from accessing this resource.');
        return EntityBlock(shortlink);
    }
    return null;
}
...
export async function EntityBotCheck(shortlink, visitorIp) {
    const botList = await loadList("entity.bots");
    const botSet = new Set(botList);

    if (botSet.has(visitorIp)) {
        // return EntityBlock(shortlink, 'Your IP is listed in our Bot blocklist.');
        return EntityBlock(shortlink);
    }
    return null;
}